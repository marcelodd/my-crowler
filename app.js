...
app.get('/pesquisar/:gtin', function (req, res) {
    pesquisar(req.params.gtin, true, function (result) {
...
function pesquisar(gtin, similares, successCallback, errorCallback) {
    if (app.get('expires_in') < new Date()) {
        var url = 'http://cosmos.bluesoft.com.br/pesquisar?utf8=true&q=' + gtin;
        request(url, function (error, response, html) {
            if (!error) {
                successCallback(tranformHTML(html, similares));
            } else {
                errorCallback(error);
            }
        });
...
function tranformHTML(html, similares) {
    var $ = cheerio.load(html);
    ...
    $(tab_content + ' #tributacao-media-' + _id + ' #states-average-taxes-pagination').children('option').each(function (i, elem) {
        ...
        _tribMedia.uf = uf;
        var tr = $('#tbl-average-taxes ' + '.state-' + uf);
        _tribMedia.aliq_nacional_fed = parseFloat(tr[0].children[3].children[0].data.replace(/\n/g, '').replace('%', ''));
        _tribMedia.aliq_importados_fed = parseFloat(tr[1].children[3].children[0].data.replace(/\n/g, '').replace('%', ''));
        _tribMedia.aliq_estadual = parseFloat(tr[2].children[3].children[0].data.replace(/\n/g, '').replace('%', ''));
        _tribMedia.chave = tr[3].children[3].children[0].data.replace(/\n/g, '');
    ...
    if (similares) {
        var gtins = buscarSimilares(_id, csrf_token, undefined, undefined, [], function (gtins) {
            pesquisar_recursivo(gtins, 0, gtins.length);
        });
    }
...
function pesquisar_recursivo(gtins, start, end) {
    pesquisar(gtins[start], false, function (produto) {
        if (start < end) {
            start++;
            pesquisar_recursivo(gtins, start, end);
        }
    }, function (error) {
...
function buscarSimilares(id, csrf_token, page_start, page_end, gtins, callback) {
    var url = 'http://cosmos.bluesoft.com.br/federal_tax_profiles/' + id + '/products';
    if (page_start)
        url = url + '?page=' + page_start.toString();
    request({
        url: url,
        headers: {
        ...
        if (!page_start) {
            var start = bodyNormalize.search('<ul');
            var end = bodyNormalize.search('</ul>');
            var ul = bodyNormalize.substring(start, end + 5);
            var $ = cheerio.load(ul);

            page_end = $('li').length;
            page_end = parseInt($('li')[page_end - 2].children[0].children[0].data);
        ...
        if (page_start <= page_end) {
            gtins = buscarSimilares(id, csrf_token, page_start, page_end, gtins, callback);
        } else {
            callback(gtins);
        }
    });
